...
def main():
    ...
    if args.send:
        if not chosen:
            # правило: событий по теме нет -> не публикуем, но уведомляем владельца в личку
            print("Событий по теме нет - публикация пропущена.")
            _dm_owner(f"Сегодня по теме «{theme.get('title','')}» событий не нашлось - "
                      "пост в каналы не публиковал.")
        else:
            try:
                import broadcast
